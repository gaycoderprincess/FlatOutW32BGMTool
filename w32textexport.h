void WriteW32MaterialsToText() {
	...
	for (auto& material : aMaterials) {
		...
		WriteFile("nShaderId: " + std::to_string(material.nShaderId) + " (" + GetShaderName(material.nShaderId, nImportFileVersion) + ")");
					...
					if (bDumpFOUCOffsetedStreams && !buf._coordsAfterFOUCMult.empty()) {
						...
						if (buf.vertexSize == 32) {
							...
							for (int k = 0; k < buf.vertexCount; k++) {
								...
								out += std::format("0x{:02X} ", value.vTangents[0]);
								out += std::format("0x{:02X} ", value.vTangents[1]);
								out += std::format("0x{:02X} ", value.vTangents[2]);
								out += std::format("0x{:02X} ", value.vTangents[3]);
								out += std::format("0x{:02X} ", value.vBitangents[0]);
								out += std::format("0x{:02X} ", value.vBitangents[1]);
								out += std::format("0x{:02X} ", value.vBitangents[2]);
								out += std::format("0x{:02X} ", value.vBitangents[3]);
...
void WriteW32TreeMeshesToText() {
	...
	for (auto& treeMesh : aTreeMeshes) {
		WriteFile("TreeMesh " + std::to_string(&treeMesh - &aTreeMeshes[0]));
		...
		if (bIsFOUCModel) {
			WriteFile("foucTrunk.nMaterialId: " + std::to_string(treeMesh.foucTrunk.nMaterialId));
			WriteFile("foucTrunk.nVertexCount: " + std::to_string(treeMesh.foucTrunk.nVertexCount));
			WriteFile("foucTrunk.nPolyCount: " + std::to_string(treeMesh.foucTrunk.nPolyCount));
			WriteFile("foucTrunk.nVertexBuffer: " + std::to_string(treeMesh.foucTrunk.nVertexBuffer));
			WriteFile("foucTrunk.nVertexOffset: " + std::to_string(treeMesh.foucTrunk.nVertexOffset));
			WriteFile("foucTrunk.nIndexBuffer: " + std::to_string(treeMesh.foucTrunk.nIndexBuffer));
			WriteFile("foucTrunk.nIndexOffset: " + std::to_string(treeMesh.foucTrunk.nIndexOffset));
			WriteFile("foucTrunk.nUnknown[0]: " + std::to_string(treeMesh.foucTrunk.nUnknown[0]));
			WriteFile("foucTrunk.nUnknown[1]: " + std::to_string(treeMesh.foucTrunk.nUnknown[1]));
			WriteFile("foucBranch.nMaterialId: " + std::to_string(treeMesh.foucBranch.nMaterialId));
			WriteFile("foucBranch.nVertexCount: " + std::to_string(treeMesh.foucBranch.nVertexCount));
			WriteFile("foucBranch.nPolyCount: " + std::to_string(treeMesh.foucBranch.nPolyCount));
			WriteFile("foucBranch.nVertexBuffer: " + std::to_string(treeMesh.foucBranch.nVertexBuffer));
			WriteFile("foucBranch.nVertexOffset: " + std::to_string(treeMesh.foucBranch.nVertexOffset));
			WriteFile("foucBranch.nIndexBuffer: " + std::to_string(treeMesh.foucBranch.nIndexBuffer));
			WriteFile("foucBranch.nIndexOffset: " + std::to_string(treeMesh.foucBranch.nIndexOffset));
			WriteFile("foucBranch.nUnknown[0]: " + std::to_string(treeMesh.foucBranch.nUnknown[0]));
			WriteFile("foucBranch.nUnknown[1]: " + std::to_string(treeMesh.foucBranch.nUnknown[1]));
			WriteFile("foucLeaf.nMaterialId: " + std::to_string(treeMesh.foucLeaf.nMaterialId));
			WriteFile("foucLeaf.nVertexCount: " + std::to_string(treeMesh.foucLeaf.nVertexCount));
			WriteFile("foucLeaf.nVertexBuffer: " + std::to_string(treeMesh.foucLeaf.nVertexBuffer));
			WriteFile("foucLeaf.nVertexOffset: " + std::to_string(treeMesh.foucLeaf.nVertexOffset));
